...
#ifndef lint
static char rcsid[] = "$Header: /home/aurora1/mark/icb/tcl/RCS/tclBasic.c,v 4.7 1994/02/16 00:40:21 mark Exp $ SPRITE (Berkeley)";
#endif /* not lint */
...
#   define NUM_ARGS 10
	...
	if ((result != TCL_OK) && (result != TCL_ERROR) && (result !=
TCL_UNKNOWN)) {
	    Tcl_Return(interp, (char *) NULL, TCL_STATIC);
	    if (result == TCL_BREAK) {
				 ...
				 * Zero means exact matches only. */
{
	...
	if (cur->name && (cur->name[0] == c) && (strcmp(cur->name, cmdName) == 0)) {
	    if (prev != NULL) {
